...
from untangle.wrappers import AdaptedLaplaceWrapper, DualLaplaceWrapper
...
def setup_devices(args: argparse.Namespace) -> tuple[torch.device, torch.device]:
    ...
    logger.info(
        f"Training on device {device}, storing eval metrics on device {storage_device}."
    )
...
def evaluate_on_test_sets(
    ...
    Raises:
        ValueError: If the combination of discarding OOD test sets is invalid.
    """
...
def update_post_hoc_method(
    ...
    Args:
        ...
        args: The command-line arguments.
    """
    if args.include_params_regex is not None:
        enable_grads_by_regex(model, args.include_params_regex)

    if isinstance(model, AdaptedLaplaceWrapper):
        model.adapted_perform_laplace_approximation(
            train_loader=train_loader,
            val_loader=hard_id_eval_loader,
            approx_method=args.approx_method,
            rank=args.rank,
        )
    elif isinstance(model, DualLaplaceWrapper):
        model.perform_dual_laplace_approximation(
            train_loader=train_loader,
            val_loader=hard_id_eval_loader,
            epistemic_params_regex=args.epistemic_params_regex,
            aleatoric_params_regex=args.aleatoric_params_regex,
            approx_method=args.approx_method,
            rank=args.rank,
        )
