...
def main() -> None:
    ...
    for j, (method_name, (imagenet_id, cifar_id)) in enumerate(
        ...
        sweep_imagenet = api.sweep(f"evelynemelanov/thesis/{imagenet_id}")
        sweep_cifar = api.sweep(f"evelynemelanov/thesis/{cifar_id}")
            ...
            for sweep, performance_matrix, prefix in [
                ...
                if metric:
                    ...
                    try:
                        ...
                        logger.info(f"KeyError: {best_key}, {metric_id}")

    logger.info(f"Perf matrix ImageNet: {performance_matrix_imagenet}")
    logger.info(f"Perf matrix CIFAR-10: {performance_matrix_cifar}")
    ...
    logger.info(f"Best methods on ImageNet: {best_methods_imagenet}")
    logger.info(f"Best methods on CIFAR-10: {best_methods_cifar}")
