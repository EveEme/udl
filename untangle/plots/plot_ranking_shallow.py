...
def setup_plot_style() -> None:
    ...
    config["figure.figsize"] = (2.64, 1.08)
...
def plot_all_methods(
    ...
    if not only_distributional:
        # Add legend for bar colors
        legend_handles = [
            Patch(facecolor=COLOR_DISTRIBUTIONAL, label="Distributional"),
            Patch(facecolor=COLOR_DETERMINISTIC, label="Deterministic"),
        ]
        ax.legend(
            frameon=False,
            handles=legend_handles,
...
def get_best_estimator(
    method: str, metric: str, metric_dict: dict[str, list[float]], decreasing: bool
) -> str:
    ...
    Args:
        ...
        metric_dict: The metric data.
        decreasing: Whether the metric is decreasing.

    Returns:
        The best estimator for the method.
    """
    method_metric_to_estimator = {
        ("Corr. Pred.", None): "error_probabilities",
        ("Loss Pred.", None): "loss_values",
        ("Mahalanobis", None): "mahalanobis_values",
        ("DDU", "auroc_oodness"): "gmm_neg_log_densities",
        ("Shallow Ens.", "auroc_oodness"): "jensen_shannon_divergences",
        ("Shallow Ens.", "rank_correlation_bregman_au"): "expected_entropies",
    }

    if metric not in ESTIMATORLESS_METRICS and metric not in CONSTRAINED_METRICS:
        # Try with specific metric first
        if (method, metric) in method_metric_to_estimator:
            return method_metric_to_estimator[method, metric]
        # Try with None (metric-independent) case
        if (method, None) in method_metric_to_estimator:
            return method_metric_to_estimator[method, None]

    reduce_operator = min if decreasing else max
    means = {key: np.mean(metric_dict[key]) for key in metric_dict if "gt" not in key}
    return reduce_operator(means.items(), key=operator.itemgetter(1))[0]
...
def plot_figures_for_dataset(
    ...
    for sweep_id, method in tqdm(id_to_method.items()):
        ...
        sweep = api.sweep(f"evelynemelanov/thesis/{sweep_id}")
        metric_dict = defaultdict(list)
    ...
    plot_all_methods(
        ...
        only_distributional=args.only_distributional,
    )
